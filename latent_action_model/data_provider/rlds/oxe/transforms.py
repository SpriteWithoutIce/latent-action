...
from data_provider.rlds.oxe.utils.droid_utils import droid_baseact_transform, droid_finetuning_transform
...
robottwin_root = Path("/home/linyihan/linyh/datasets/RoboTwin")
assert robottwin_root.exists(), f"{robottwin_root} not found"

for p in sorted(robottwin_root.iterdir()):
    if not p.is_dir():
        continue

    name = f"aloha_{p.name}"
    OXE_STANDARDIZATION_TRANSFORMS[name] = aloha_dataset_transform
